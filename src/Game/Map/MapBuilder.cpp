    ...
    for (int i = 0; i < mapImage.getWidth(); i++) {
        for (int j = 0; j < mapImage.getHeight(); j++) {
            ...
            if(currentPixel == boundBoundBlock){
                ...
                Player* PacMan = new Player(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager, true);
				mapInCreation->setPlayer(PacMan);
				Player* SANS = new Player(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager, false);
				mapInCreation->setPlayer2(SANS);
				mapInCreation->setPlayer3(PacMan);
				...
				float tNum = ofRandom(99);
				if(tNum < 3){
					if(tNum>0 && tNum<=0.999){
				...
				mapInCreation->setDotCount(mapInCreation->getdotcount()+1);
			}
